...
MARKDOWN_FILENAME = "PROXIES.md"  # Markdown output filename
...
def write_markdown_footer(md_file):
    """
    Writes the footer section with notes and attribution.
    ...
    md_file.write("## 📝 Notes\n\n")  # Write notes header
    md_file.write("- All proxies are scraped from free public sources\n")  # Write note about source
    md_file.write("- Proxy availability and functionality are not guaranteed\n")  # Write disclaimer
    md_file.write("- This list is updated automatically every 12 hours\n")  # Write update frequency
    md_file.write("- For raw text files, check the `Proxies_List/` directory\n\n")  # Write location of text files
    
    md_file.write("---\n\n")  # Write separator
    md_file.write("*Generated by [Proxy List Generator](https://github.com/BrenoFariasdaSilva/Proxy-List-Generator)*\n")  # Write attribution


def generate_markdown_proxy_list(proxies_dict):
    """
    Generates a markdown file with the collected proxies organized by source.
    
    Orchestrates the creation of a well-formatted markdown document by delegating
    to specialized helper functions for each section of the document.
    
    :param proxies_dict: Dictionary with website names as keys and proxy lists as values
    :return: None
    """

    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")  # Get current timestamp
    total_proxies = sum(len(proxies) for proxies in proxies_dict.values())  # Calculate total proxy count
    
    verbose_output(
        f"{BackgroundColors.GREEN}Generating markdown proxy list with {total_proxies} proxies...{Style.RESET_ALL}"
    )  # Output generation message
    
    with open(MARKDOWN_FILENAME, "w", encoding="utf-8") as md_file:  # Open markdown file in write mode
        ...
        for source_key in sorted(proxies_dict.keys()):  # Iterate through sorted source keys
            ...
            if proxies:  # Only write section if proxies exist
                write_markdown_proxy_section(md_file, source_key, proxies)  # Write proxy section
        
        write_markdown_footer(md_file)  # Write footer section
    
    verbose_output(
        f"{BackgroundColors.GREEN}Markdown proxy list generated: {BackgroundColors.CYAN}{MARKDOWN_FILENAME}{Style.RESET_ALL}"
